...
Timestamp parse_timestamp_field_or_default(const Json& j, const char* key) {
  if (!j.contains(key)) {
    return {};
  }
...
std::vector<std::string> parse_conditions(const Json& j, const char* key) {
...
StreamMessage build_trade_message(const Json& payload) {
...
StreamMessage build_quote_message(const Json& payload) {
...
StreamMessage build_bar_message(const Json& payload) {
  BarMessage message{};
  message.symbol = payload.value("S", "");
  message.timestamp = parse_timestamp_field_or_default(payload, "t");
...
StreamMessage build_status_message(const Json& payload) {
...
StreamMessage build_error_message(const std::string& message) {
  return ErrorMessage{message};
}

StreamMessage build_error_message(const Json& payload) {
...
StreamMessage build_control_message(const Json& payload, std::string type) {
  return ControlMessage{std::move(type), payload};
}

StreamMessage build_order_update(const Json& payload) {
...
StreamMessage build_account_update(const Json& payload) {
...
WebSocketClient::WebSocketClient(std::string url, std::string key, std::string secret,
                                 StreamFeed feed)
...
void WebSocketClient::subscribe(const MarketSubscription& subscription) {
  MarketSubscription diff;
  {
    std::lock_guard<std::mutex> lock(connection_mutex_);
    for (const auto& symbol : subscription.trades) {
      if (subscribed_trades_.insert(symbol).second) {
        diff.trades.push_back(symbol);
      }
    }
    for (const auto& symbol : subscription.quotes) {
      if (subscribed_quotes_.insert(symbol).second) {
        diff.quotes.push_back(symbol);
      }
    }
    for (const auto& symbol : subscription.bars) {
      if (subscribed_bars_.insert(symbol).second) {
        diff.bars.push_back(symbol);
      }
    }
    for (const auto& symbol : subscription.statuses) {
  ...
  if (diff.trades.empty() && diff.quotes.empty() && diff.bars.empty() &&
      diff.statuses.empty()) {
    return;
  }
...
void WebSocketClient::unsubscribe(const MarketSubscription& subscription) {
  MarketSubscription diff;
  {
    std::lock_guard<std::mutex> lock(connection_mutex_);
    for (const auto& symbol : subscription.trades) {
...
void WebSocketClient::listen(const std::vector<std::string>& streams) {
...
void WebSocketClient::send_raw(const Json& message) {
  std::lock_guard<std::mutex> lock(connection_mutex_);
  if (!connected_) {
  ...
  const auto info = socket_.sendText(payload);
...
void WebSocketClient::handle_payload(const Json& payload) {
  if (!message_handler_) {
    return;
  }

  if (payload.contains("T")) {
    const auto type = payload.at("T").get<std::string>();
    if (type == "t") {
    ...
    if (type == "success" || type == "subscription" || type == "cancel" ||
        type == "control" || type == "ping") {
  ...
  if (payload.contains("stream")) {
    const auto stream = payload.at("stream").get<std::string>();
    ...
    if (stream == "account_updates") {
      if (payload.contains("data")) {
        message_handler_(build_account_update(payload.at("data")),
                         MessageCategory::AccountUpdate);
      }
      return;
    }
  ...
  if (payload.contains("event")) {
    const auto event = payload.at("event").get<std::string>();
...
void WebSocketClient::handle_control_payload(const Json& payload, const std::string& type) {
  if (type == "ping") {
    Json response;
...
void WebSocketClient::schedule_reconnect() {
  ...
  const std::size_t capped_attempt = std::min<std::size_t>(attempt - 1, 5);
  const auto delay_seconds = static_cast<unsigned>(1u << capped_attempt);
