...
std::optional<std::string> optional_string_from_any(const Json& j, const char* key) {
  if (!j.contains(key) || j.at(key).is_null()) {
    return std::nullopt;
  }
...
void from_json(const Json& j, NewsImage& image) {
  j.at("url").get_to(image.url);
  image.caption = optional_string(j, "caption");
  image.size = optional_string(j, "size");
}

void from_json(const Json& j, NewsArticle& article) {
  article.raw = j;
  if (j.contains("id") && !j.at("id").is_null()) {
    const auto id_value = optional_string_from_any(j, "id");
...
void from_json(const Json& j, NewsResponse& response) {
