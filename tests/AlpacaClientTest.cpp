...
class StubHttpClient : public alpaca::HttpClient {
 public:
  ...
  void enqueue_response(alpaca::HttpResponse response) { responses_.push(std::move(response)); }

  [[nodiscard]] const std::vector<alpaca::HttpRequest>& requests() const { return requests_; }

  alpaca::HttpResponse send(const alpaca::HttpRequest& request) override {
...
TEST(AlpacaClientTest, SubmitOrderSerializesAdvancedFields) {
  ...
  const alpaca::Order order = client.submit_order(request);
...
TEST(AlpacaClientTest, CalendarRequestsIncludeQueryParameters) {
  ...
  stub->enqueue_response(alpaca::HttpResponse{200,
                                              R"([{"date":"2023-01-03","open":"09:30","close":"16:00"}])",
                                              {}});
  ...
  const auto calendar = client.get_calendar(request);
...
TEST(AlpacaClientTest, LatestStockTradeUsesMarketDataEndpoint) {
  auto stub = std::make_shared<StubHttpClient>();
  stub->enqueue_response(alpaca::HttpResponse{200,
                                              R"({"symbol":"AAPL","trade":{"i":"t1","x":"P","p":123.45,"s":10,"t":"2023-01-01T00:00:00Z"}})",
                                              {}});

  alpaca::Configuration config = alpaca::Configuration::Paper("key", "secret");
  alpaca::AlpacaClient client(config, stub);

  const auto latest = client.get_latest_stock_trade("AAPL");
...
TEST(AlpacaClientTest, StockBarsForwardQueryParameters) {
  auto stub = std::make_shared<StubHttpClient>();
  stub->enqueue_response(alpaca::HttpResponse{200,
                                              R"({"symbol":"AAPL","bars":[{"t":"2023-01-01T00:00:00Z","o":1.0,"h":1.5,"l":0.5,"c":1.2,"v":100}]})",
                                              {}});
  ...
  const auto bars = client.get_stock_bars("AAPL", request);
...
TEST(AlpacaClientTest, GetAllStockBarsTraversesPagination) {
  ...
  const auto bars = client.get_all_stock_bars("AAPL", request);
...
TEST(AlpacaClientTest, DeleteBrokerAccountUsesCloseActionEndpoint) {
  ...
  ASSERT_EQ(stub->requests().size(), 1U);
  const auto& request = stub->requests().front();
