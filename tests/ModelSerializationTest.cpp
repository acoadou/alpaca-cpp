...
TEST(ModelSerializationTest, AccountDeserializesExtendedFields) {
  ...
  const auto account = json.get<alpaca::Account>();
...
TEST(ModelSerializationTest, PortfolioHistoryParsesNumericVectors) {
  alpaca::Json json = {{"timestamp", alpaca::Json::array({1, 2})},
  ...
  const auto history = json.get<alpaca::PortfolioHistory>();
...
TEST(ModelSerializationTest, WatchlistSerializationIncludesSymbols) {
  alpaca::CreateWatchlistRequest request{"My Watchlist", {"AAPL", "MSFT"}};
  alpaca::Json json;
  to_json(json, request);
...
TEST(ModelSerializationTest, LatestStockTradeParsesCoreFields) {
  ...
  const auto latest = json.get<alpaca::LatestStockTrade>();
...
TEST(ModelSerializationTest, StockBarsParseOptionalFields) {
  alpaca::Json json = {{"symbol", "AAPL"},
                       {"bars",
                        alpaca::Json::array({{{"t", "2023-01-01T14:30:00Z"},
                                              {"o", 120.0},
                                              {"h", 121.0},
                                              {"l", 119.5},
                                              {"c", 120.5},
  ...
  const auto bars = json.get<alpaca::StockBars>();
...
TEST(ModelSerializationTest, StockSnapshotHandlesMissingAggregates) {
  alpaca::Json json = {{"symbol", "MSFT"},
                       ...
                       {"minuteBar", nullptr},
                       {"dailyBar", nullptr},
                       {"prevDailyBar", {{"t", "2022-12-30T21:00:00Z"},
                                          {"o", 240.0},
                                          {"h", 242.0},
                                          {"l", 238.0},
                                          {"c", 241.5},
  ...
  const auto snapshot = json.get<alpaca::StockSnapshot>();
  ...
  EXPECT_EQ(snapshot.previous_daily_bar->timestamp,
            alpaca::parse_timestamp("2022-12-30T21:00:00Z"));
...
TEST(ModelSerializationTest, NewsResponseParsesArticles) {
  alpaca::Json json = {{"news",
                        alpaca::Json::array({{{"id", 1234},
                                               {"headline", "Headline"},
                                               {"author", "Reporter"},
                                               {"summary", "Summary"},
                                               ...
                                               {"images", alpaca::Json::array({{{"url", "https://img"},
                                                                                {"caption", "Caption"},
                                                                                {"size", "large"}}})},
  ...
  const auto response = json.get<alpaca::NewsResponse>();
  ASSERT_EQ(response.news.size(), 1U);
  const auto& article = response.news.front();
...
TEST(ModelSerializationTest, CorporateActionAnnouncementsParseCoreFields) {
  alpaca::Json json = {{"announcements",
                        alpaca::Json::array({{{"id", "ann-1"},
  ...
  const auto response = json.get<alpaca::CorporateActionAnnouncementsResponse>();
  ASSERT_EQ(response.announcements.size(), 1U);
  const auto& announcement = response.announcements.front();
...
TEST(ModelSerializationTest, CorporateActionEventsParseCoreFields) {
  alpaca::Json json = {{"events",
                        alpaca::Json::array({{{"id", "evt-1"},
                                              {"corporate_action_id", "ca-1"},
                                              {"ca_type", "cash_dividend"},
                                              {"ca_sub_type", "regular_cash"},
                                              {"symbol", "AAPL"},
  ...
  const auto response = json.get<alpaca::CorporateActionEventsResponse>();
  ASSERT_EQ(response.events.size(), 1U);
  const auto& event = response.events.front();
...
TEST(ModelSerializationTest, MultiStockResponsesMapSymbols) {
  alpaca::Json json = {{"bars",
                        {{"AAPL",
                          alpaca::Json::array({{{"t", "2023-01-01T00:00:00Z"},
                                                {"o", 1.0},
                                                {"h", 2.0},
                                                {"l", 0.5},
                                                {"c", 1.5},
                                                {"v", 1000}}})},
                         {"MSFT",
                          alpaca::Json::array({{{"t", "2023-01-01T00:01:00Z"},
                                                {"o", 2.0},
                                                {"h", 2.5},
                                                {"l", 1.5},
                                                {"c", 2.2},
