...
TEST(ApiExceptionTest, ParsesDeltaSecondsRetryAfter) {
  std::unordered_map<std::string, std::string> headers{{"Retry-After", "5"}};
...
TEST(ApiExceptionTest, ParsesHttpDateRetryAfter) {
  const auto target = std::chrono::system_clock::now() + std::chrono::seconds{10};
  ...
  std::unordered_map<std::string, std::string> headers{{"Retry-After", header_value.str()}};
