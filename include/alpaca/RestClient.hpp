...
class RestClient {
 public:
  ...
  template <typename T>
  T get(const std::string& path, const QueryParams& params = {}) {
  ...
  T del(const std::string& path, const QueryParams& params = {}) {
    return request_json<T>(HttpMethod::DELETE_, path, params, std::nullopt);
  }

  /// Performs a POST request with a JSON payload and returns the JSON response as \c T.
  template <typename T>
  T post(const std::string& path, const Json& payload, const QueryParams& params = {}) {
    return request_json<T>(HttpMethod::POST, path, params, payload.dump());
  }

  /// Performs a PUT request with a JSON payload and returns the response as \c T.
  template <typename T>
  T put(const std::string& path, const Json& payload, const QueryParams& params = {}) {
    return request_json<T>(HttpMethod::PUT, path, params, payload.dump());
  }

  /// Performs a PATCH request with a JSON payload and returns the response as \c T.
  template <typename T>
  T patch(const std::string& path, const Json& payload, const QueryParams& params = {}) {
 ...
 private:
  ...
  static std::string build_url(const std::string& base, const std::string& path, const QueryParams& params);
  static std::string encode_query(const QueryParams& params);

  std::optional<Json> perform_request(HttpMethod method, const std::string& path, const QueryParams& params,
                                      std::optional<std::string> payload);

  template <typename T>
  T request_json(HttpMethod method, const std::string& path, const QueryParams& params,
                 std::optional<std::string> payload) {
    ...
    if (!json.has_value()) {
      if constexpr (std::is_void_v<T>) {
        ...
        throw ApiException(204, "Empty response body cannot be deserialized into requested type", std::string{}, {});
      }
    }
