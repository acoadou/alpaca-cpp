...
/// Market status or halt/resume notification delivered through market data streams.
...
/// Error message emitted either by the server or by the websocket implementation.
struct ErrorMessage {
  std::string message;
};

/// Control-plane message delivered by Alpaca (success, subscription, ping, etc.).
...
using StreamMessage = std::variant<TradeMessage, QuoteMessage, BarMessage, StatusMessage,
                                   OrderUpdateMessage, AccountUpdateMessage, ErrorMessage,
                                   ControlMessage>;
...
using MessageHandler = std::function<void(const StreamMessage&, MessageCategory)>;

/// Callback invoked for lifecycle events such as the connection opening or closing.
using LifecycleHandler = std::function<void()>;

/// Callback invoked when the websocket stack reports an error.
using ErrorHandler = std::function<void(const std::string&)>;

/// Lightweight websocket client capable of connecting to Alpaca's streaming APIs.
class WebSocketClient {
 public:
  WebSocketClient(std::string url, std::string key, std::string secret,
                  StreamFeed feed = StreamFeed::MarketData);
  ~WebSocketClient();
  ...
  void unsubscribe(const MarketSubscription& subscription);

  /// Subscribe to trading stream channels (e.g. "trade_updates", "account_updates").
  void listen(const std::vector<std::string>& streams);

  void send_raw(const Json& message);
 ...
 private:
  void authenticate();
  void handle_payload(const Json& payload);
  void handle_control_payload(const Json& payload, const std::string& type);
